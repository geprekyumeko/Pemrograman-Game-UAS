...
{
    [Header("Health")]
    public int maxHealth = 3; // Maximum health of the Raja Iblis
    public GameObject explosionEffectPrefab; // Prefab for explosion effect


    public Transform player;
    public float patrolSpeed = 2.5f; // Speed at which the Raja Iblis patrols
    public float attackRange = 5f; // Range within which the Raja Iblis can attack
    public float chaseSpeed = 3f; // Speed at which the Raja Iblis chases the player
        ...
        if (Vector2.Distance(transform.position, player.position) <= attackRange)
            ...
            else
            {
                ...
                if (hit == false)
                {
                    if (facingLeft == true)
                    {
                        transform.eulerAngles = new Vector3(0f, -180f, 0f); // Flip the Raja Iblis to face right
                        facingLeft = false; // Update facing direction
                    }
                    else
                    {
                        transform.eulerAngles = new Vector3(0f, -0f, 0f); // Flip the Raja Iblis to face right
    ...
    public void TakeDamage(int damage)
    {
        if (maxHealth <= 0)
        {
            Debug.Log("Raja Iblis is already dead.");
            return; // Exit if the Raja Iblis is already dead
        }
        maxHealth -= damage;
    }
